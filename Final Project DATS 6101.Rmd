...
author: "Alex Zakrzeski, Aditya Nayak, Purvi Jain, Tracie Robinson, and Alejandra Mejia"
output:
  html_document:
    code_folding: hide
...
findings offer a clearer understanding of which characteristics of vehicles
are the most responsible for high CO2 emissions, therefore making individuals
more likely to choose to drive eco-friendly vehicles to slow climate change.
...
Three ANOVA models are built.Under the first model, we expect to understand if 
CO2 emissions are different among the numbers of cylinders in a car and its fuel
type. Therefore, Model 1,  is a two-way ANOVA that displays CO2 Emissions as a 
function of Number of Cylinders and Fuel Type. At the 1% significance level, 
Number of Cylinders is statistically significant. Then, at the 5% significance
level, Fuel Type is statistically significant. The model's residual variance is 
7,400,174.From this output, we can see that Cylinders explain a significant 
amount of variation in average CO2 emissions with p values less than 0.001, 
while Fuel type even though still significant explains in a smaller proportion 
the variation in average CO2 emissions. Moreover, we are able to see that the 
residual met the assumption of normality.
...
plot(density(twa1$residuals))
#code to create a Q-Q plot
qqnorm(twa1$residuals)
qqline(twa1$residuals, datax = FALSE, distribution = qnorm, probs=c(0.25, 0.75))

```

We have a reason to think that two of your independent variables have an 
interaction effect rather than an additive effect. Therefore, Model 2, the 
second ANOVA model, is a two-way ANOVA that displays CO2 Emissions as a function
of the interaction between Number of Cylinders and Fuel Type. At the 1% 
significance level, Number of Cylinders and the interaction between Number of 
Cylinders and Fuel Type are statistically significant. At the 5% significance 
level, Fuel Type is statistically significant. The model's residual variance is
6,995,135, which is lower than that of the first ANOVA model.We can also see 
that the ‘Cylinders:Fuel_Type:density’ variable has a high sum-of-squares value
and a low p value, which means there is a greater variation that can be 
explained by the interaction between cylinders and fuel type.
...
Additionally to using McFadden as evaluation tool we also performed the Hosmer and Lemeshow Goodness of Fit test
```{r HosmerLemeshow}

library("ResourceSelection") 
glmLogitHoslem = hoslem.test(data$Bin_Em, fitted(glm1))
glmLogitHoslem
glm2LogitHoslem = hoslem.test(data$Bin_Em, fitted(glm2))
glm2LogitHoslem

```
The p-value of `r glmLogitHoslem$p.value` and `r glm2LogitHoslem$p.value` is 
fairly small. This could indicate our models are not really a good fit, 
despite all the coefficients are significant. Notwithstanding, it is worth 
mentioning that since the inception of the Hosmer and Lemeshow Goodness of Fit 
test, many statistical books do not recommend to use it, given it multiple 
flaws like: not taking into account overfitting and its tendency to have low 
power, its little guidance on selecting the number of subgroups and its 
arbitrary choices of bins. For this reasons, and based on our additional
analysis we considered our models are performing fairly well. 
...
## VI. References
* Viollet, O. (2019). Predict cars’ CO2 emissions (v1.0). https://rpubs.com. https://rpubs.com/Olivier_Viollet_2019/CO2_emissions
* M. Singh and R. Dubey, "Deep Learning Model Based CO2 Emissions Prediction using Vehicle Telematics Sensors Data," in IEEE Transactions on Intelligent Vehicles, doi: 10.1109/TIV.2021.3102400.
* N. Subramaniam and N. Yusof(2021). "Modelling of CO2 Emission Prediction for Dynamic Vehicle Travel Behavior Using Ensemble Machine Learning Technique," 2021 IEEE 19th Student Conference on Research and Development (SCOReD), pp. 383-387, doi: 10.1109/SCOReD53546.2021.9652757.
* Kakouei, Aliakbar & Vatani, Ali & Idris, Ahmad. (2012). An estimation of traffic related CO2 emissions from motor vehicles in the capital city of, Iran. Iranian journal of environmental health science & engineering. 9. 13. 10.1186/1735-2746-9-13. 
* Anderson, M. L., and M. Auffhammer, 2014: Pounds that kill. Rev. Econ. Stud., 82, 35–571.
* Y. Zhang (2021), "Prediction of Esports Game Results Using Early Game Datasets," ICMLCA 2021; 2nd International Conference on Machine Learning and Computer Application, pp. 1-6
* S. Lee-Cultura, K. Sharma, S. Papavlasopoulou and M. Giannakos (2020), "Motion-Based Educational Games: Using Multi-Modal Data to Predict Player’s Performance," 2020 IEEE Conference on Games (CoG), pp. 17-24, doi: 10.1109/CoG47356.2020.9231892.
* S. K. Nivetha, M. Geetha, R. C. Suganthe, R. M. Prabakaran, S. Madhuvanan and A. M. Sameer (2022), "A Deep Learning Framework for Football Match Prediction," 2022 International Conference on Computer Communication and Informatics (ICCCI), pp. 1-7, doi: 10.1109/ICCCI54379.2022.9740760.
* C. Pipatchatchawal and S. Phimoltares (2021), "Predicting Football Match Result Using Fusion-based Classification Models," 2021 18th International Joint Conference on Computer Science and Software Engineering (JCSSE), pp. 1-6, doi: 10.1109/JCSSE53117.2021.9493837.
* Paruchuri Vik (2012).Predicting the NBA Finals with R. https://www.vikparuchuri.com/predicting-nba-finals-with-r/
* Herb Wilson (2018). Learning R in R Studio: Predicting Baseball Winning Percentages [Video]. YouTube. https://www.youtube.com/watch?v=rQosPYLDZpg
*  Hickman, Robert (2019). An Introduction to Modelling Soccer Matches in R (part 1).
https://www.r-bloggers.com/2019/05/an-introduction-to-modelling-soccer-matches-in-r-part-1/
